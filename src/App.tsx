...
const sampleQuestions: Question[] = [
  ...
  },
  {
    id: '231',
    type: QuestionType.MSQ,
    question: 'An ecommerce company is collecting structured data and unstructured data from its website, mobile apps, and IoT devices. The data is stored in several databases and Amazon S3 buckets. The company is implementing a scalable repository to store structured data and unstructured data. The company must implement a solution that provides a central data catalog, self-service access to the data, and granular data access policies and encryption to protect the data.\n\nWhich combination of actions will meet these requirements with the LEAST amount of setup? (Choose three.)',
    options: [
      { id: '231a', text: 'Identify the existing data in the databases and S3 buckets. Link the data to AWS Lake Formation.', isCorrect: true },
      { id: '231b', text: 'Identify the existing data in the databases and S3 buckets. Link the data to AWS Glue.', isCorrect: false },
      { id: '231c', text: 'Run AWS Glue crawlers on the linked data sources to create a central data catalog.', isCorrect: true },
      { id: '231d', text: 'Apply granular access policies by using AWS Identity and Access Management (IAM). Configure server-side encryption on each data source.', isCorrect: false },
      { id: '231e', text: 'Apply granular access policies and encryption by using AWS Lake Formation.', isCorrect: true },
      { id: '231f', text: 'Apply granular access policies and encryption by using AWS Glue.', isCorrect: false },
    ],
    explanation: 'AWS Lake Formation can register existing S3 data and external data sources, use AWS Glue crawlers to build a central catalog, and provide fine-grained permissions and encryption management. The least-setup path is to register the existing data with Lake Formation (A), crawl it with Glue (C), and then use Lake Formation\'s built-in access control and encryption features (E).',
  },
  {
    id: '232',
    type: QuestionType.MSQ,
    question: 'A machine learning (ML) specialist is developing a deep learning sentiment analysis model that is based on data from movie reviews. After the ML specialist trains the model and reviews the model results on the validation set, the ML specialist discovers that the model is overfitting.\n\nWhich solutions will MOST improve the model generalization and reduce overfitting? (Choose three.)',
    options: [
      { id: '232a', text: 'Shuffle the dataset with a different seed.', isCorrect: false },
      { id: '232b', text: 'Decrease the learning rate.', isCorrect: false },
      { id: '232c', text: 'Increase the number of layers in the network.', isCorrect: false },
      { id: '232d', text: 'Add L1 regularization and L2 regularization.', isCorrect: true },
      { id: '232e', text: 'Add dropout.', isCorrect: true },
      { id: '232f', text: 'Decrease the number of layers in the network.', isCorrect: true },
    ],
    explanation: 'Overfitting in a deep network is mitigated by reducing model capacity or constraining weights. Adding L1/L2 regularization (D) penalizes large weights, dropout (E) randomly drops units during training to improve generalization, and simplifying the architecture by reducing layers (F) lowers model complexity, all of which help reduce overfitting.',
  },
  {
    id: '233',
    type: QuestionType.MCQ,
    ...
    options: [
      { id: '233a', text: 'Retrain the model with L1 regularization applied.', isCorrect: true },
      { id: '233b', text: 'Retrain the model with L2 regularization applied.', isCorrect: false },
      { id: '233c', text: 'Retrain the model with dropout regularization applied.', isCorrect: false },
      { id: '233d', text: 'Retrain the model by using more data.', isCorrect: false },
    ],
    explanation: 'L1 regularization (Lasso) adds an absolute value penalty that drives many coefficients exactly to zero, performing feature selection and reducing model complexity. This both combats overfitting and reduces the number of active features while keeping a linear model suitable for low-latency prediction.',
  },
  {
    id: '234',
    type: QuestionType.MCQ,
    ...
    options: [
      { id: '234a', text: 'Apply the Synthetic Minority Oversampling Technique (SMOTE) on the minority class in the training dataset. Retrain the model with the updated training data.', isCorrect: true },
      { id: '234b', text: 'Apply the Synthetic Minority Oversampling Technique (SMOTE) on the majority class in the training dataset. Retrain the model with the updated training data.', isCorrect: false },
      { id: '234c', text: 'Undersample the minority class.', isCorrect: false },
      { id: '234d', text: 'Oversample the majority class.', isCorrect: false },
    ],
    explanation: 'Fraud detection is a highly imbalanced classification problem where the fraud class is rare but critical. Applying SMOTE to the minority (fraud) class (A) synthesizes additional fraud examples, helping the model better learn the minority class decision boundary and improve recall without discarding majority-class data.',
  },
  {
    id: '235',
    type: QuestionType.MCQ,
    question: 'A company is training machine learning (ML) models on Amazon SageMaker by using 200 TB of data that is stored in Amazon S3 buckets. The training data consists of individual files that are each larger than 200 MB in size. The company needs a data access solution that offers the shortest processing time and the least amount of setup.\n\nWhich solution will meet these requirements?',
    options: [
      { id: '235a', text: 'Use File mode in SageMaker to copy the dataset from the S3 buckets to the ML instance storage.', isCorrect: false },
      { id: '235b', text: 'Create an Amazon FSx for Lustre file system. Link the file system to the S3 buckets.', isCorrect: false },
      { id: '235c', text: 'Create an Amazon Elastic File System (Amazon EFS) file system. Mount the file system to the training instances.', isCorrect: false },
      { id: '235d', text: 'Use FastFile mode in SageMaker to stream the files on demand from the S3 buckets.', isCorrect: true },
    ],
    explanation: 'SageMaker FastFile mode streams data directly from S3 to training instances without first copying the full dataset to local storage. It provides high-throughput, low-latency access for large files with minimal configuration effort compared to setting up and linking external file systems.',
  },
  {
    id: '75',
    type: QuestionType.MCQ,
